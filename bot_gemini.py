...
API_KEY = "AIzaSyCrdz6AKwJSL3lIHxbchh2C0zg6UYTXNKg"
...
def main():
    ...
    while True:
        ...
        if user_input.lower().startswith("salva "):
            ...
            continue
        notes_block = "\n".join(f"- {n}" for n in _NOTE_STORE) if _NOTE_STORE else "(nessuno)"
        prompt = TEMPLATE.format(system=_CFG, notes=notes_block, user_input=user_input)
        try:
